<template>
    <div>
        ...
        <div style="margin-top:10px;">
            <h4>Grid #2</h4>
            <grid-layout :layout="layout2" :col-num="12" :row-height="30" :is-draggable="true" :is-resizable="true"
                :vertical-compact="true" :use-css-transforms="true">
                <grid-item v-for="(item, index) in layout2" :x="item.x" :y="item.y" :w="item.w" :h="item.h" :i="item.i" :key="index">
                    <span class="text">{{ item.i }}</span>
                </grid-item>
            </grid-layout>
        </div>
