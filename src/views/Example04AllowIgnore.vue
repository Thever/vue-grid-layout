    ...
    <div>
        <grid-layout :layout.sync="layout" :col-num="12" :row-height="30" :is-draggable="true" :is-resizable="true"
            ...
            <grid-item v-for="(item, index) in layout" :x="item.x" :y="item.y" :w="item.w" :h="item.h" :i="item.i"
                drag-allow-from=".vue-draggable-handle" drag-ignore-from=".no-drag" :key="index">
                <!-- drag-allow-from=".vue-draggable-handle" 些子元素可以触发拖拽事件，值为css-like选择器
                drag-ignore-from=".no-drag" 标识栅格元素中哪些子元素无法触发调整大小的事件，值为css-like选择器 -->
                <div class="text">
